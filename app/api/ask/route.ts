...
Use prior turns to interpret follow-ups, but judge only the current excerpt list.

Rules:
- Hard limit: ${ASK_LIMITS.maxAnswerChars} characters for your answer (the CITED: line does not count).
...
${excerptsBlock}`;
  ...
  let answer = enforceAnswerLength(raw.trim());

  if (citedMatch) {
    answer = enforceAnswerLength(raw.slice(citedMatch[0].length).trim());
